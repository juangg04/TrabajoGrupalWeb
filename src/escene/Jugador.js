...
class Jugador extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y) {
        ...
        scene.anims.create({
            key: 'hit',
            frames: scene.anims.generateFrameNumbers('player', { start: 13, end: 19 }),
    ...
    update(cursors) {
        ...
        if (!this.isHitting) {
            if (cursors.left.isDown) {
                ...
                if (this.body.onFloor() && (!this.anims.isPlaying || this.anims.currentAnim.key !== 'walk')) {
                ...
                this.setVelocityX(0);
                if (this.body.onFloor() && (!this.anims.isPlaying || this.anims.currentAnim.key !== 'idle')) {
